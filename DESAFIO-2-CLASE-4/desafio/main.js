...
class Container {
    ...
    async save( element ) {
        try {
            const response = await fs.promises.readFile(`./${this.file}.txt`, 'utf-8');
            ...
            try {
                await fs.promises.writeFile(`./${this.file}.txt`, JSON.stringify(productList,null,2));
            ...
            try {
                await fs.promises.writeFile(`./${this.file}.txt`,JSON.stringify([product],null,2));
                return product.id;
            } catch (err) {
                return {
                    status: 'Error', 
    ...
    async deleteById( id ) {
        try{
            ...
            try {
                await fs.promises.writeFile(`./${this.file}.txt`,JSON.stringify(updateProducts,null,2));
                return {
                    status: 'Success', 
    ...
    async deleteAll() {
        try{
            const filesDeleted = [];
            await fs.promises.writeFile(`./${this.file}.txt`,JSON.stringify(filesDeleted,null,2))
